...
2. `tools/`: attack count is 1, covering TL-01. Models are `anthropic/claude-sonnet-4-5`, `ollama/llama3.1:8b`, and `google/gemini-3.5-flash` for the agentic claim (`groq/llama-3.3-70b-versatile` excluded). Required `n` is 20 per completed model for the anchored agentic run (seed 0). Done means `RESULTS.md` has ASR and TR rows for Sonnet and Llama at n=20 with Wilson intervals; Gemini may be `---` if the provider run is unavailable (do not invent zeros); each row names the Inspect log. Historical diagnostic n=3 rows may remain but are not the citation anchor.
