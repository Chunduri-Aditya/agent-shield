...
unchanged. Groq remains excluded from the **anchored agentic** claim
(`docs/powered_tools_tr_design.md`).

### tools/ anchored n=20 (agentic) — Wilson 95% CI

Pre-registered: TL-01 only, Inspect tool loop, seed 0, n=20
(`tools_asr_anchored` / `tools_transparency_anchored`). Groq excluded.
...
agentic TR claim. Silent non-disclosure holds at n=20 for Sonnet and Llama
(ASR=TR=0).

### tools_asr — Targeted ASR per model (diagnostic n=3)
